\section{Specifications}\todo{change}
...
\begin{itemize}
    \item ESP32 microcontroller as the central processing unit.
    \item Integration of the DHT22 temperature and humidity sensor.
    \item Connection and calibration of the Load Cell with the HX711 amplifier for weight measurement.
    \item Acquisition of \gls{RFID} with the MFRC522 module.
    \item Open and close the door via SG90-180 servomotors.
    \item Implementation of a \gls{RGB} \gls{LED} for status indication.
    \item Connection with ThingsBoard \gls{IoT} platform for data visualization and remote monitoring with \gls{MQTT}.
    \item Arduino microcontroller for 5V power supply.
    \item Arduino \gls{IDE} for prototyping and sensor testing.
\end{itemize}
