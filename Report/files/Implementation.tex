...
\begin{table}[H]
    ...
    \label{tab:libraries}
    \begin{tabular}{p{0.17\textwidth}p{0.4\textwidth}p{0.2\textwidth}}
        \toprule
        \textbf{Library} & \textbf{Function} & \textbf{Author/Maintainer} \\ 
        \midrule
        WiFi.h and WiFiClientSecure & Wireless connectivity and \gls{TLS} encryption & Espressif Systems \\ 
        PubSubClient & \gls{MQTT} protocol implementation & Nick O'Leary \\ 
        ArduinoJson & \gls{JSON} serialization for telemetry & Benoit Blanchon \\ 
        ESP32Servo & \gls{PWM} control for SG90 motors & Kevin Harrington \\ 
...
\begin{figure}[H]
    ...
    \caption{ThingsBoard Shared Attributes configuration for \acrshort{NEST} devices}
